...
<h1>Can You Make The Math Work? (Single Number)</h1>
<br>
</div>
</div>
<div class="row">
...
echo "<h4>Total number of combinations = " . (sizeof($finalarray[$length-2])-1) . "</h4>";
?>
		...
		<?php
			...
			for ($i=1; $i < sizeof($finalarray[$length-2], 1); $i++){
				if ($finalarray[$length-2][$i] == $numberarray[$length-1]){
					echo "<tr><td>Yes</td><td>" . $equationarray[$length-2][$i] . "</td><td>" . $finalarray[$length-2][$i] . "</td></tr>";
					$yesnumber++;
				}
				else {
					echo "<tr><td>No</td><td>" . $equationarray[$length-2][$i] . "</td><td>" . $finalarray[$length-2][$i] . "</td></tr>";
				}
			}
	}
	else {
