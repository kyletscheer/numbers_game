...
<h1>Can You Make The Math Work? (Bulk Numbers)(PEMDAS Respected)</h1>
<br>
</div>
</div>
<div class="row">
<div class="col-lg-12 text-center">
<form method="POST" action="numbers6.php">
...
array_push($equationarray[1], ($numberarray[0] . "/" . $numberarray[1]));
	...
	$combinationsarray[$k] = (sizeof($equationarray[$length-2])-1);
	$yesnumber = 0;
	...
	<tbody>
		<?php
			for ($i=1; $i < sizeof($optionsarray); $i++){
					echo "<tr><td><a href='/numbers5.php?sent=" . $optionsarray[$i] . "' target='_blank'>" . $optionsarray[$i] . "</a></td><td>" . $yesnumberarray[$i] . "</td><td>" . $combinationsarray[$i] . "</td></tr>";					
				}
		?>
	</tbody>
</table>
